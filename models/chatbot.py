...
# Importer la fonction retrieve_with_threshold de manière plus robuste
try:
    from search import retrieve_with_threshold
    print("✓ Importation réussie de retrieve_with_threshold")
except ImportError as e:
    print(f"⚠️ Erreur lors de l'importation de retrieve_with_threshold: {e}")
    # Définir une fonction de secours
    from langchain_core.documents import Document
    
    def retrieve_with_threshold(query, threshold=0.4):
        """Fonction de secours si la vraie fonction n'est pas disponible"""
        print("⚠️ Utilisation de la fonction retrieve_with_threshold de secours dans chatbot.py")
        dummy_doc = Document(
            page_content=f"Ceci est un document de secours pour la requête : {query}",
            metadata={"score": 0.8, "id": "fallback-doc"}
        )
        return [dummy_doc]
...
# Par cette configuration fixe:
os.environ["TAVILY_API_KEY"] = "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"
